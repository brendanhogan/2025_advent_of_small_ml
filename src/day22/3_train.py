"""
Step 3: Train Monad on pre-tokenized SYNTH dataset with weighted loss.

Uses loss_weights from tokenization to weight <think> tokens less than answer tokens.
...
class WeightedLossTrainer(Trainer):
    """Trainer that computes per-token loss weights on-the-fly."""
    
    def __init__(self, *args, think_start_ids=None, think_end_ids=None, think_weight=0.5, **kwargs):
        super().__init__(*args, **kwargs)
        # Token IDs for <think> and </think> - computed once from tokenizer
        self.think_start_ids = think_start_ids or []
        self.think_end_ids = think_end_ids or []
        self.think_weight = think_weight
    
    def _compute_weights(self, input_ids: torch.Tensor) -> torch.Tensor:
        """Compute loss weights on-the-fly by finding <think>...</think> spans."""
        ...
        start_len = len(self.think_start_ids)
        end_len = len(self.think_end_ids)
        start_tensor = torch.tensor(self.think_start_ids, device=input_ids.device)
        end_tensor = torch.tensor(self.think_end_ids, device=input_ids.device)
        ...
        for b in range(batch_size):
            ...
            while i < seq_len:
                ...
                if i <= seq_len - start_len and torch.equal(ids[i:i+start_len], start_tensor):
                    in_think = True
                    weights[b, i:i+start_len] = self.think_weight
                    i += start_len
                # Check for </think> end
                elif i <= seq_len - end_len and torch.equal(ids[i:i+end_len], end_tensor):
                    weights[b, i:i+end_len] = self.think_weight
                    in_think = False
                    i += end_len
                else:
                    if in_think:
                        weights[b, i] = self.think_weight
                    i += 1
    ...
    def compute_loss(self, model, inputs, return_outputs=False, num_items_in_batch=None):
        ...
        # Remove pre-computed weights if present (backwards compat), we compute on-the-fly
        inputs.pop("loss_weights", None)
        
        # Compute weights on-the-fly from input_ids
        loss_weights = self._compute_weights(inputs["input_ids"])
        
        # Debug: check weights look reasonable (should have mix of 0.5 and 1.0)
        if not hasattr(self, '_weight_debug_done'):
            unique_weights = torch.unique(loss_weights)
            think_ratio = (loss_weights == self.think_weight).float().mean().item()
            print(f"[DEBUG] Weight stats: unique={unique_weights.tolist()}, think_ratio={think_ratio:.2%}")
            self._weight_debug_done = True
        ...
        shift_weights = loss_weights[..., 1:].contiguous().to(logits.device)
        ...
        # Apply weights: mask invalid labels (< 0) and apply loss_weights
        valid_mask = (shift_labels.view(-1) >= 0).float()
        weighted_loss = loss * shift_weights.view(-1) * valid_mask
        
        # Average over valid tokens
        denom = (shift_weights.view(-1) * valid_mask).sum()
        
        # Debug: check loss components
        if not hasattr(self, '_loss_debug_done'):
            wsum = weighted_loss.sum().item()
            final = wsum / denom.item()
            print(f"[DEBUG] Loss stats: raw_mean={loss.mean().item():.4f}, valid_tokens={valid_mask.sum().item():.0f}, denom={denom.item():.2f}, weighted_sum={wsum:.2f}, final_loss={final:.4f}")
            self._loss_debug_done = True
...
def train(
    ...
    print("=" * 60)
    print("Monad Training (weighted loss)")
    print("=" * 60)
    ...
    # Get token IDs for <think> and </think> (for on-the-fly weight computation)
    ...
    print(f"  <think> token IDs: {think_start_ids}")
    print(f"  </think> token IDs: {think_end_ids}")
    ...
    if accelerator.is_main_process:
        ...
        if use_wandb:
            wandb.init(
                project="monad_training",
                name=f"monad_{timestamp}",
            )
    ...
    args = TrainingArguments(
        ...
        average_tokens_across_devices=False,  # We compute our own weighted mean
        seed=seed,
