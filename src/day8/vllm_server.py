...
def sanitize_logprob(logprob):
    ...
    if math.isnan(value) or math.isinf(value):
        logger.warning(f"Generated invalid logprob (NaN/Inf), token logprob '{logprob}' will be set to -1000.0")
        return -1000.0  # Return a large negative number instead of -inf for JSON compatibility
...
def main(script_args: ScriptArguments):
    ...
    async def get_tensor_parallel_size():
        """
        Retrieves the tensor parallel size of the LLM engine.
