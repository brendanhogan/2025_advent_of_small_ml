...
def load_checkpoint(checkpoint_path, model_name, device, model_type="base"):
    ...
    # Load model state dict (check for both old and new format)
    if model_type == "base":
        model_path = os.path.join(checkpoint_path, "base_model.pt")
        if not os.path.exists(model_path):
            # Fallback to old format
            model_path = os.path.join(checkpoint_path, "model.pt")
    else:
        model_path = os.path.join(checkpoint_path, "adversary_model.pt")
    ...
    if os.path.exists(model_path):
        ...
        print(f"Warning: {model_type} model not found in {checkpoint_path}, using base model")
