...
def format_prompt(system_prompt: str, question: str, tokenizer) -> Tuple[str, torch.Tensor, torch.Tensor]:
    ...
    inputs = tokenizer(prompt_text, return_tensors="pt", padding=True, add_special_tokens=True)
    return prompt_text, inputs["input_ids"], inputs["attention_mask"]


def extract_answer(text: str) -> str:
    """
    Extract answer from after </think> tag (Baguettotron format).
    
    Baguettotron was trained with format:
        <think>reasoning</think>
        answer
    
    So we extract everything after </think> up to end or next special token.
    """
    import re
    
    # Look for content after </think>
    # Match </think> followed by optional whitespace, then capture until end or <|im_end|>
    pattern = r"</think>\s*(.+?)(?:<\|im_end\|>|<\|endoftext\|>|$)"
...
def check_format(text: str) -> float:
    """
    Check if text contains <think></think> tags (Baguettotron format).
    
    Returns 0.2 if correct format, -0.5 if wrong format.
    Note: Baguettotron doesn't use <answer> tags - answer comes after </think>.
    """
    import re
    
    # Check for think tags (required)
