...
class VLLMClient:
    ...
    def __init__(
        self, host: str = "0.0.0.0", server_port: int = 8000, group_port: int = 51216, connection_timeout: float = 0.0, base_url: str = None
    ):
        ...
        # Don't auto-initialize communicator - let the caller do it with device info
    ...
    def generate(
        ...
        Args:
            ...
            seed (`int` or `None`, *optional*, defaults to `None`):
                Random seed for deterministic generation. If not provided, generation will be non-deterministic.

        Returns:
            `dict`:
                Dictionary containing:
                - `prompt_ids`: List of lists of token IDs for each input prompt
                - `completion_ids`: List of lists of token IDs for each generated completion
        ...
        if seed is not None:
            payload["generation_kwargs"] = {"seed": seed}
    ...
    def init_communicator(self, device=None):
        ...
        Args:
            device: The device to use for communication (e.g., "cuda:0" or torch.device("cuda:0"))
        """
        if device is None:
            device = "cuda:0"
        ...
        # Get device UUID for the server
        import torch
        ...
        response = self.session.post(url, json={
            ...
            "client_device_uuid": device_uuid
        })
        if response.status_code != 200:
            raise Exception(f"Request failed: {response.status_code}, {response.text}")
        ...
        self.pynccl_comm = PyNcclCommunicator(pg, device=device) # type: ignore
    ...
    def update_named_param(self, name: str, weights: torch.Tensor):
        ...
        # Ensure weights are on the same device as the communicator
        if weights.device != self.pynccl_comm.device:
            weights = weights.to(self.pynccl_comm.device)
