"""
Step 3: Train Monad on pre-tokenized SYNTH dataset with NEFTune.

NEFTune adds noise to embeddings during training to reduce overfitting.
Paper: "NEFTune: Noisy Embeddings Improve Instruction Finetuning" (ICLR 2024)

Requires: tokenized_synth/ directory from 2_tokenize.py (symlinked from day20)
...
def train(
    ...
    neftune_noise_alpha: float = 5.0,  # NEFTune noise scale (paper uses 5-15)
    # Checkpointing
    ...
    print("=" * 60)
    print("Monad Training with NEFTune")
    print("=" * 60)
    print(f"NEFTune noise alpha: {neftune_noise_alpha}")
    ...
    run_dir = os.path.join(output_dir, f"monad_neftune_{timestamp}")
    ...
    if accelerator.is_main_process:
        ...
        if use_wandb:
            wandb.init(
                ...
                name=f"monad_neftune_{timestamp}",
                config={"neftune_noise_alpha": neftune_noise_alpha},
            )
